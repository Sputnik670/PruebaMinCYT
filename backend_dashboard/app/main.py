...
# Aquí estaba el error. He corregido la URL de Render y asegurado el dominio de Vercel.
origins = [
    "http://localhost:5173",
    "http://127.0.0.1:5173",
    "https://prueba-mincyt.onrender.com",  # <--- CORREGIDO (tenía una 'i' extra y faltaba el guion)
    "https://pruebasmincyt.ar",
    "https://www.pruebasmincyt.ar",
    "https://pruebamincyt.vercel.app",     # Agregado por seguridad (dominio corto)
...
@app.get("/")
def read_root():
    return {"mensaje": "API Activa v2"} # Cambié el mensaje para que sepas cuando se actualizó
...
def obtener_tendencia_inversion():
    try:
        ...
        # Ajusta los nombres si tus columnas en Google Sheets se llaman distinto
        # Ejemplo: Si en el Sheet dice 'Inversion' (sin tilde), cambia 'Inversión' abajo.
